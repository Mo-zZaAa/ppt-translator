...
예를 들어, "앰버서더 2기 김세은"이라는 말을 처리한다고 한다면,
PPT 내부에서는 "앰버서더 2기 김세은" 한 문장이 실제로 이렇게 쪼개져 저장됩니다:
...
이때 run 단위로 번역하면 "기" 혼자 → "Issue", "2" → "2" 처럼 맥락 없이 깨집니다.
그러나 paragraph 단위로 합쳐서 번역하면 전체 문맥이 유지됩니다!
